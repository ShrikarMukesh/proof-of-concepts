...
public class ReadVersion4 {
	...
	public static void main (String []args) throws IOException, NoSuchAlgorithmException, InvalidKeySpecException {
		...
		CredentialOptions co = CredentialOptions.credential(sac);
		
		BigtableOptions op = BigtableOptions.builder().setCredentialOptions(co).build();
		try (BigtableSession session = new BigtableSession(op)) {
			
			//BigtableDataClient bt = BigtableDataClient.
